...
function App(): JSX.Element {
  ...
  async function linkAccount(
    ...
    try {
      ...
      if (result.status === 204) {
        toast.error(
          'ไม่พบบัญชีนี้ในอมูเลตดีดี โปรดกรอกเบอร์มือถือในบัญชีอมูเลตดีดีของท่าน',
          { id: toastId }
        )
        return
      }
      if (result?.status === 200) {
        ...
        liff.sendMessages([
          {
            type: 'text',
            text: 'คุณได้เชื่อมต่อบัญชี Line กับ อมูเลตดีดี เรียบร้อย 😊'
          }
        ])
      } else {
  ...
  async function unlinkAccount(
    ...
    try {
      ...
      if (result.status === 204) {
        toast.error('ไม่พบบัญชีนี้ในอมูเลตดีดี')
        return
      }
      if (result?.status === 200) {
        ...
        liff.sendMessages([
          {
            type: 'text',
            text: 'คุณได้ยกเลิกเชื่อมต่อบัญชี Line กับ อมูเลตดีดี 😔'
          }
        ])
      } else {
